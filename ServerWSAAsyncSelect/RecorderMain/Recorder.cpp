...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		std::shared_ptr<RecorderServer> recorderServer;
		std::thread t1([&recorderServer, &host, &fromTo, &threads, &destFolderW]() {
			//Create Window and MessageLoop must be on the same thread
			recorderServer = std::make_shared<RecorderServer>();
