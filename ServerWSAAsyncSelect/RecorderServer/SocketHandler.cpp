...
void SocketHandler::CreateSocket(const std::string &host, int16_t portNumber, HWND hwnd)
{
	...
	if (WSAAsyncSelect(listenSocket, hwnd, WM_SOCKET, FD_READ | FD_WRITE)) {
		auto iResult = WSAGetLastError();
		throw std::runtime_error("WSAAsync Select failed. Error:" + std::to_string(iResult));
	}
